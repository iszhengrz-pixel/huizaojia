...
interface User {
  ...
  role: string;
  gender: string;
  avatar: string | 'light' | 'placeholder' | null;
}

const MOCK_USERS: User[] = [
  { id: '1', username: 'admin', nickname: '管理员', role: '管理员,测试1', gender: '男', avatar: 'light' },
              ...
              {MOCK_USERS.map((user) => (
                <tr key={user.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    {user.avatar === 'light' || user.avatar === 'placeholder' || !user.avatar ? (
          ...
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-[500px] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
            ...
            <div className="flex-1 overflow-y-auto p-8 flex flex-col items-center">
              ...
              >
                ...
                />
                <div className="w-24 h-24 bg-slate-100 border border-slate-200 rounded-[28px] overflow-hidden flex items-center justify-center transition-transform group-hover:scale-105 active:scale-95 duration-200 shadow-md">
                  {currentUser.avatar === 'light' || currentUser.avatar === 'placeholder' || !currentUser.avatar ? (
                    <Icon name="User" size={32} className="text-slate-400" />
                  ) : (
                    <img src={currentUser.avatar} alt="avatar" className="w-full h-full object-cover" />
                  )}
                  {/* 悬停遮罩 */}
                  <div className="absolute inset-0 bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center rounded-[28px]">
                    <Icon name="Camera" size={20} className="text-slate-500 mb-1" />
                    <span className="text-slate-600 text-[10px] font-bold">更换头像</span>
                  </div>
                </div>
              </div>
